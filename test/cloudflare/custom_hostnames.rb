...
require "cloudflare/custom_hostnames"
require "cloudflare/a_connection"

describe Cloudflare::CustomHostnames do
	include_context Cloudflare::AConnection

	let(:domain) {"www-#{job_id}.#{zone_name}"}
	let(:record) {zone.custom_hostnames.create(domain)}

	let(:subdomain) {"origin-#{job_id}"}
	let(:dns_record) {zone.dns_records.create("A", subdomain, "1.2.3.4")}
	
	let(:custom_origin) {"#{subdomain}.#{zone.name}"}
	...
	# 	expect(record).to be_a Cloudflare::CustomHostname
	# 	expect(record.custom_metadata).to be_nil
	# 	expect(record.hostname).to be == domain
	# 	expect(record.custom_origin).to be_nil
	# 	expect(record.ssl.method).to be == "http"
	# 	expect(record.ssl.type).to be == "dv"
	# end
	...
	# 			expect(record.result[:ssl].values_at(:method, :http_body, :http_url).compact).not_to be_empty
	# 			expect(record.ssl.method).to be record.result[:ssl][:method]
	# 			expect(record.ssl.http_body).to be record.result[:ssl][:http_body]
	# 			expect(record.ssl.http_url).to be record.result[:ssl][:http_url]
	# 		end
	...
	# 			expect(record.ssl.method).not_to be_nil
	# 			expect(record.ssl.type).not_to be_nil
	# 			record.result[:ssl][:status] = "pending_validation"
